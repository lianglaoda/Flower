...
Mock.mock('http://localhost:8080/api/list', 'post', (params) => {
  let dataList=[],listObj
  for(let i=0;i<30;i++){
    let listObj={
      title:Random.csentence(5,10),//随机生成一段中文文本
      des:Random.csentence(5,10),
      number:Random.integer(100,9999)  //返回一个随机的整数
    }
  ...
  var len = total / info.size

  var totalPages = len - parseInt(len) > 0 ? parseInt(len) + 1 : len



  var newDataList = dataList.slice((info.index-1) * info.size, info.index  * info.size)
